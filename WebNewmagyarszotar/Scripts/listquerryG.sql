...
cast(ISNULL(thing.reaction,'N') as VARCHAR)as reaction
INTO #TEMP
FROM angolszo 
...
LEFT join (select * from Reactions where felhasz_id= @felhaszId)as thing on magyarszo.id=thing.magyarszo_id
